...
namespace mp4box
{
    ...
    public partial class WorkingForm : Form
    {
        ...
        /// <summary>
        /// Duration in seconds of current processing media.
        /// </summary>
        ...
        public static class Patterns
        {
            ...
            /// </summary>
            private const string ffmpegRegStr = @"\bDuration: (?<duration>\d{2}:\d{2}:\d{2}\.\d{2})";
            ...
            /// <summary>
            /// ffmpeg input info output regex. Match common frame rate tags in ffmpeg -i output.
            ///     Available patterns: rate.
            /// </summary>
            private const string ffmpegRateRegStr = @"\bVideo:.*?(?<rate>\d+\.?\d+)\s(?:tbr|fps)\b";
            public static readonly Regex ffmpegRateReg = new Regex(ffmpegRateRegStr, RegexOptions.Singleline);

            /// <summary>
            /// ffmpeg running output sample:
            /// <para>frame=  248 fps=... time=00:00:10.34 bitrate=...</para>
            /// </summary>
            private const string ffmpegFrameRegStr = @"\bframe=\s*(?<frame>\d+)\b";
            public static readonly Regex ffmpegFrameReg = new Regex(ffmpegFrameRegStr);

            private const string ffmpegTimeRegStr = @"\btime=(?<time>\d{2}:\d{2}:\d{2}\.\d{2})\b";
            public static readonly Regex ffmpegTimeReg = new Regex(ffmpegTimeRegStr);
        }
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                savDlg.FileName = "LogFile-" + DateTime.Now.ToString("yyyy'-'MM'-'dd'_'HH'-'mm'-'ss") + ".log";
        ...
        private void OutputHandler(object sender, System.Diagnostics.DataReceivedEventArgs e)
        {
                    ...
                    progressBarX264.InvokeIfRequired(() =>
            {
                ...
                progressBarX264.Value = 0;
            });
            bgworker.ReportProgress(0, 0.0);
                    mediaDurationSeconds = 0;
                    frameCount = EstimateFrame(workPath, result.Groups["fileIn"].Value, out mediaDurationSeconds);
                }
                ...
                if (result.Success)
                    bgworker.ReportProgress(0,
                        Double.Parse(result.Groups["percent"].Value, CultureInfo.InvariantCulture) / 100);
                else
                {
                    // try ffmpeg time pattern first (most reliable for current ffmpeg builds)
                    result = Patterns.ffmpegTimeReg.Match(e.Data);
                    if (result.Success && mediaDurationSeconds > 0)
                    {
                        var encodedSeconds = TimeSpan.Parse(result.Groups["time"].Value).TotalSeconds;
                        bgworker.ReportProgress(0, encodedSeconds / mediaDurationSeconds);
                        return;
                    }

                    // try ffmpeg frame pattern
                    result = Patterns.ffmpegFrameReg.Match(e.Data);
                    if (result.Success && frameCount > 0)
                    {
                        bgworker.ReportProgress(0,
                            Double.Parse(result.Groups["frame"].Value, CultureInfo.InvariantCulture) / frameCount);
                        return;
                    }
        ...
        private void UpdateProgress(double value)
        {
            ...
            notifyIcon.Text = "岚珠工具箱" + Environment.NewLine +
        ...
        private int EstimateFrame(string workPath, string filePath, out double durationSeconds)
        {
            durationSeconds = 0;
            ...
            durationSeconds = TimeSpan.Parse(result.Groups["duration"].Value).TotalSeconds;
            var rateResult = Patterns.ffmpegRateReg.Match(mediaInfo);
            if (!rateResult.Success)
                return 0;

            // add a 1% tolerance to avoid unintentional overflow on progress bar
            return Convert.ToInt32(durationSeconds
                * Double.Parse(rateResult.Groups["rate"].Value, CultureInfo.InvariantCulture) * 1.01);
        }
        ...
        private void BalloonTip(string notes, int timeout = 500)
        {
            ...
            {
                notifyIcon.Visible = true;
                notifyIcon.ShowBalloonTip(timeout, "岚珠工具箱", notes, ToolTipIcon.Info);
            }
        }
